...
| `seo.ts` | REWRITE | Share metadata for `spa.vanityvice.blog`: titles, descriptions, canonical origin, and the WordPress-hosted share image. Values RETAIN; emission moves to the canonical head/meta layer. |
...
| `DeskLayout.tsx` | REWRITE | Shell: masthead, chapter breaks, navigation, language and theme controls, venue bar placement. Composition RETAIN, framework wiring REWRITE. |
...
| `src/routes/__root.tsx` | REWRITE (metadata only) | Document shell, font links, and the emitted share meta, including the WordPress-hosted `og:image` / `twitter:image` override. Take the metadata values; leave the runtime shell behind. |
...
| lockfile, config, `robots.txt`, `public/og/*` | REJECT | Canonical owns these. The live share image is the WordPress URL, not the local card. |
...
`ef43d0b` and what to leave behind; the merge itself happens in the canonical
repository.
