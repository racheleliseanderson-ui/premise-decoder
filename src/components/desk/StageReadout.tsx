...
export function StageReadout({
  ...
  return (
    <section className="no-print border border-rule bg-bone/70 grain" aria-label="Where you stand">
      ...
      <ScrollRail
        label="Six live readings"
        hint={`${stages.length} steps in this row · scrolls sideways`}
      >
        <ol className="flex snap-x snap-mandatory">
          {stages.map((s) => (
            <li
              key={s.def.id}
            ...
            >
              ...
              >
                {/* Six of these sit on one screen. The visible words stay the same
                  for scanning; the accessible name says which step it opens. */}
                Open this step
                <span className="sr-only"> · {s.def.name}</span>
              </button>
            </li>
          ))}
        </ol>
